...
import 'package:notes_app/services/gemini_api_service.dart';
...
void main() {
  ...
  setUp(() async {
    tempDir = await Directory.systemTemp.createTemp('gemini_api_service_test_');
  ...
  group('isVerified', () {
    ...
    test('isVerified_verifiedFlagTrue_returnsTrue', () async {
      await storage.write('gemini_api_key_verified', 'true');
  ...
  group('clearKey', () {
    test('clearKey_withStoredKey_removesKeyAndVerifiedFlag', () async {
      await storage.write('gemini_api_key', 'AIza-test');
      await storage.write('gemini_api_key_verified', 'true');
      await service.clearKey();
      expect(await storage.read('gemini_api_key'), isNull);
      expect(await storage.read('gemini_api_key_verified'), isNull);
    });
  });
  ...
  group('activateKey', () {
    ...
    test('activateKey_validKey_savesKeyAndReturnsNull', () async {
      ...
      final result = await service.activateKey('AIza-valid');
      expect(result, isNull);
      expect(await storage.read('gemini_api_key'), 'AIza-valid');
      expect(await storage.read('gemini_api_key_verified'), 'true');
    });

    test('activateKey_invalidKey_returnsErrorAndDoesNotSave', () async {
      service.setClient(MockClient((_) async => http.Response('', 401)));
      final result = await service.activateKey('AIza-bad');
    ...
    test('activateKey_trimsWhitespace_savesCleanKey', () async {
      service.setClient(MockClient((_) async =>
          http.Response(jsonEncode(_successBody()), 200)));
      await service.activateKey('  AIza-trimmed  ');
      expect(await storage.read('gemini_api_key'), 'AIza-trimmed');
    });
  });
  ...
  group('rewriteNote', () {
    ...
    test('rewriteNote_success_returnsRewrittenText', () async {
      await storage.write('gemini_api_key', 'AIza-valid');
      service.setClient(MockClient((_) async =>
    ...
    test('rewriteNote_401Response_throwsAuthAndClearsVerifiedFlag', () async {
      ...
      expect(await storage.read('gemini_api_key_verified'), 'false');
    });
